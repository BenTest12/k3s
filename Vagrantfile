  ...
  (1..3).each do |i|
    config.vm.define "k3s-server-#{i}" do |server|
      ...
      server.vm.provision "shell", inline: <<-SHELL
        ...
        sudo firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" source address="192.168.33.11" accept' --permanent
        sudo firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" source address="192.168.33.12" accept' --permanent
        sudo firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" source address="192.168.33.13" accept' --permanent
        sudo firewall-cmd --zone=public --add-rich-rule='rule family="ipv4" source address="192.168.33.14" accept' --permanent
